...
int main(int argc, char **argv)
{
	...
	printf("This is a simple program to perform an image transform.\r\n");
...
int inputCheck(void)
{
	char *cmds[] = { "help", "grayscale", "bright", "sharp", "exit"};         
	...
	//char allocInput[6]; /*For re-alloc question*/
     ...
     compare = strcmp(userInput,"");
     while(compare == 32)      /* Validating user input, No input = 'SPACE' (i.e decimal value 32)*/
      {
        int value = 0;
...
/****************************************** Parsing End **************************************************/
	int valid = 0;
			...
			else if (strcmp(Token[0], cmds[1]) == 0)             /*     grayscale()     */
			{
				valid = 0;
				if (Token[1] == NULL || Token[2] == NULL)
				{
                    printf("Please enter a valid resolution. Type <help> for details\n");
                    printf("Enter another command: \r\n");
					return valid;
                }	
				 else if((atoi(Token[1]) >= 0 && (atoi(Token[1]) <= 1280)) || \
				 (atoi(Token[2] >= 0) && (atoi(Token[2] <= 960))))             
                    {
                        printf("Lock to grayscale thread\n");
						HRES = atoi(Token[1]);
						VRES = atoi(Token[2]);
						printf("Resolution is: %d x %d\n", HRES, VRES);
						return valid;
				...
				else if (strcmp(Token[0], cmds[2]) == 0)             /*     bright()     */	
			{		
				valid = 0;
				 if(Token[1] == NULL)             
                    {
                        printf("Please enter a valid input. Type <help> for details\n");
                        printf("Enter another command: \r\n");
						return valid;
                    }
				else
				...
				else if (strcmp(Token[0], cmds[3]) == 0)             /*     sharp()     */	
			{		
				valid = 0;
				 if(Token[1] == NULL || Token[2] == NULL)             
                    {

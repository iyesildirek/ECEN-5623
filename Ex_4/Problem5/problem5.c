...
void *fib10(void *threadp)
{
   ...
   required_test_cycles = (int)(1.0/run_time);
...
void *fib20(void *threadp)
{
   ...
   required_test_cycles = (int)(2.0/run_time);
...
void main(void)
{
   ...
   CPU_COUNT(&allcpuset));
    ...
    //
    if (sem_init (&semF10, 0, 0)) { printf \
	("Failed to initialize semF10 semaphore\n"); exit (-1); }
    if (sem_init (&semF20, 0, 0)) { printf \
	("Failed to initialize semF20 semaphore\n"); exit (-1); }

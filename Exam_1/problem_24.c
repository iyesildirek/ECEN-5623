...
* $ gcc problem_24.c -o b.out -lpthread -Wall
...
void *incThread(void *threadp)
{
	...
	syslog(LOG_CRIT,"The precision value for x is: %0.2f and y is: %0.2f\n",rt_precision.x, rt_precision.y);
...
void *decThread(void *threadp)
{
	...
	syslog(LOG_CRIT,"RT clock start seconds = %ld, nanoseconds = %ld\n", 
         rtclk_start_time.tv_sec, rtclk_start_time.tv_nsec);	 
		 
	//printf("RT clock stop seconds = %ld, nanoseconds = %ld\n", 
    //     rtclk_stop_time.tv_sec, rtclk_stop_time.tv_nsec);
	syslog(LOG_CRIT,"RT clock stop seconds = %ld, nanoseconds = %ld\n", 
         rtclk_stop_time.tv_sec, rtclk_stop_time.tv_nsec);
	...
	syslog(LOG_CRIT,"RT clock delta seconds = %ld, nanoseconds = %ld\n", 
         (rtclk_stop_time.tv_sec - rtclk_start_time.tv_sec), \
		 (rtclk_stop_time.tv_nsec - rtclk_start_time.tv_nsec));	 
...
int main (int argc, char *argv[])
{
   ...
   if (rc)
   {
       ...
       syslog(LOG_CRIT,"ERROR; sched_setscheduler rc is %d\n", rc);

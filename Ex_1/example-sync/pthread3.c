...
/**
* @file pthread3.c
* @brief This source file contains a c program that has four 
* threads plus main. Starts by checking input for number of 
* secods and then set the thread attributes like scheduler 
* to FIFO. Then it starts services and assigning priorties 
* based on the FIFO max index. Pthread join is used to run 
* each service and wait until the service is complete to go 
* to the next one and do the same. This code also logs the 
* time of start and stop for each thread and uses a semaphore 
* to control msg resource. This is done using mutex semaphore.
*

...
* - example-sync/deadlock_timeout.c
* Compile code by using the gcc command below:
* $ gcc deadlock_timeout.c -o3 a.out -lpthread -Wall
...
* racing condition of two threads trying to get resource A.
* whichever thread get resource A is able to run to completion
* prior to the other thread to start. 
*
...
void *grabRsrcs(void *threadid)
{
   ...
   {
     printf("THREAD 2 grabbing resource A @ %d sec and %d nsec\n", timeNow.tv_sec, timeNow.tv_nsec);
     ...
     printf("THREAD 2 got A, trying for B @ %d sec and %d nsec\n", timeNow.tv_sec, timeNow.tv_nsec);
     rc=pthread_mutex_timedlock(&rsrcB, &rsrcB_timeout);
     //rc=pthread_mutex_lock(&rsrcA);
     if(rc == 0)
     {
         clock_gettime(CLOCK_REALTIME, &timeNow);
         printf("Thread 2 GOT B @ %d sec and %d nsec with rc=%d\n", timeNow.tv_sec, timeNow.tv_nsec, rc);
         rsrcBCnt++;
         printf("rsrcACnt=%d, rsrcBCnt=%d\n", rsrcACnt, rsrcBCnt);
     }
     else if(rc == ETIMEDOUT)
